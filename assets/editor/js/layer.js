pg.layer = function () {
    ...
    var initImage = function () {
        p = getUrlParameter("name")
        $.getJSON("/api/v1/ImageREST/" + p, function (data) {
            var dataFacade = JSON.parse(data)

            if (dataFacade.description) {
                pg.document.loadJSONDocument(dataFacade.description)
            }

            pg.import.importAndAddExternalImage(dataFacade.url) // http://localhost:63342/papergrapher-master/index.html?path=https://sun9-18.userapi.com/c628827/v628827039/2ae3b/MxUAtng7lBQ.jpg

            pg.undo.onUpdate = function () {
                var outd = pg.document.JSONDocument()
                $.ajax({
                    url: "/api/v1/ImageREST/" + p,
                    type: 'POST',
                    dataType: "json",
                    contentType: "application/json; charset=utf-8",
                    data: JSON.stringify({"description": outd})
                });
            }
        });
    };
