...
run pip install Pillow email-validator fastcore flask-admin flask_login flask_sqlalchemy flask_user==1.* flask_compress flask_httpauth flask_restplus protobuf flask_dropzone flask_uploads flask-triangle  flask_restful  flask-locale flask-cors Flask-Caching Flask-BabelEx flask_babel transliterate && pip install Werkzeug==0.16.*
